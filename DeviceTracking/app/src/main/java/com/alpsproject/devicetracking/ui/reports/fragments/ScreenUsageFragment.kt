...
class ScreenUsageFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(R.layout.fragment_screen_usage, container, false)
        initChart(view)
        return view
    }

    private fun initChart(view: View) {
        val screenUsageChart: AnyChartView = view.findViewById(R.id.screen_usage_chart)
        val screenUsageProgress: ProgressBar = view.findViewById(R.id.screen_usage_progress_bar)
        screenUsageChart.setProgressBar(screenUsageProgress)

        val cartesian: Cartesian = AnyChart.column()
        val data: MutableList<DataEntry> = ArrayList()
        val dummyData = DummyDataGenerator.generateUsageHours(90)
        ...
        screenUsageChart.setChart(cartesian)
    }
